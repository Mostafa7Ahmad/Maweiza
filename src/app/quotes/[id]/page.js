...
export default function _({ params }) {
    ...
    const showData = filterData.map((item, index) => (
        <div
            ...
            className={
                "px-6 py-6 mb-3" +
                (filterData.length - 1 === index ? "" : " border-b border-stone-700 border-solid")
            }>
            <figure className="max-w-screen-md mx-auto text-center">
                ...
                <blockquote><p className="text-2xl italic font-medium">{item.text}</p></blockquote>
                <p className="mt-5 block pr-3 font-medium">{item.author}</p>
            </figure>
