...
export default async function AlQaran({ params }) {
    ...
    const showData = Surs.map((item, key) =>
        <Link key={key} href={`/qaran/memorizing/${idRecitations}/${item.number}`} className="flex bg-white dark:bg-black dark:hover:bg-lime-600 items-center hover:bg-lime-600 hover:text-white transition-colors flex-row justify-between p-5 border-2 border-solid border-lime-600">
