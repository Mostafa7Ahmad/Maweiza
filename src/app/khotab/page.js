...
export default async function Hhotba() {
    let khotab = [];

    try {
        const response = await fetch('https://api3.islamhouse.com/v3/paV29H2gm56kvLPy/main/khotab/ar/ar/1/10000/json')
                    ...
                    <>
                        <span key={key2}>
                            <a href={item2.url} download className="flex justify-between mt-2 w-full items-center px-3 py-2 text-sm font-medium text-center text-white bg-lime-700 rounded-lg hover:bg-lime-800">
                                <span>  تحميل الخطبه بصيغه {item2.extension_type}</span>
